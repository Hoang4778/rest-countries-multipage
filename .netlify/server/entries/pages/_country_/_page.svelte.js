...
<div id="${"container"}" class="${"svelte-8zxrbv"}"><a id="${"back"}" href="${"/"}" class="${"svelte-8zxrbv"}"><span class="${"material-symbols-outlined"}">keyboard_backspace </span>
    ...
    <div id="${"info"}" class="${"svelte-8zxrbv"}"><img${add_attribute("src", country.flags.svg, 0)}${add_attribute("alt", country.name.common + " flag", 0)} class="${"svelte-8zxrbv"}">
        <div><p id="${"title"}" class="${"svelte-8zxrbv"}">${escape(country.name.common)}</p>
            <div id="${"desc"}" class="${"svelte-8zxrbv"}"><p class="${"svelte-8zxrbv"}"><span class="${"svelte-8zxrbv"}">Native Name:</span><span class="${"svelte-8zxrbv"}">${escape(nativeNames)}</span></p>
                ...
                <p class="${"svelte-8zxrbv"}"><span class="${"svelte-8zxrbv"}">Currencies:</span><span class="${"svelte-8zxrbv"}">${escape(currencies ? currencies : "N/A")}</span></p>
                <p class="${"svelte-8zxrbv"}"><span class="${"svelte-8zxrbv"}">Region:</span><span class="${"svelte-8zxrbv"}">${escape(country.region)}</span></p>
                <p class="${"svelte-8zxrbv"}"><span class="${"svelte-8zxrbv"}">Languages:</span><span class="${"svelte-8zxrbv"}">${escape(languages ? languages : "N/A")}</span></p>
                <p class="${"svelte-8zxrbv"}"><span class="${"svelte-8zxrbv"}">Sub Region:</span><span class="${"svelte-8zxrbv"}">${escape(country.subregion ? country.subregion : "N/A")}</span></p>
                <p class="${"svelte-8zxrbv"}"><span class="${"svelte-8zxrbv"}">Capital:</span><span class="${"svelte-8zxrbv"}">${escape(country.capital ? country.capital : "N/A")}</span></p></div></div>
    ...
    return `<li class="${"svelte-8zxrbv"}"><a href="${"/border/" + escape(nation, true)}" class="${"svelte-8zxrbv"}">${escape(nation)}</a>
                        </li>`;
